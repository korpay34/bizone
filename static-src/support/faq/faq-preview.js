...
form.addEventListener('submit',async event=>{event.preventDefault();const status=form.querySelector('.inquiry-form-status'),submit=form.querySelector('[type="submit"]');submit.disabled=true;status.className='inquiry-form-status';status.textContent='문의글을 등록하고 있습니다.';try{const response=await fetch('/api/inquiries',{method:'POST',body:new FormData(form)}),result=await response.json();if(!response.ok)throw new Error(result.error||'문의글을 등록하지 못했습니다.');status.className='inquiry-form-status success';status.textContent='문의글이 등록되었습니다.';location.href=`/support/faq/${result.id}`}catch(error){status.className='inquiry-form-status error';status.textContent=error.message;submit.disabled=false}});
